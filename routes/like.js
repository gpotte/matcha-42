router.post('/like', middleware.loggedIn(), (req, res)=>{
  var currentUser = {username: req.cookies.user.username, photo: req.cookies.user.photo},
      ...
      room = [username, currentUser.username].sort();
  profile.toArray().then((profile)=>{
    ...
    //LIKE
      req.app.db.collection("users").update({username: username, 'like.name': {$ne: currentUser.username}}, {$addToSet: {like: {name: currentUser.username, date: dateFormat(), time: Date.now(), type: "like", photo: currentUser.photo}}, $inc: {fame: 2}});
    //MATCH
    if (profile[0].like){
    var i = profile[0].like.length;
    while(i-- > 0) {
      if (profile[0].like[i].name === username){
        req.app.db.collection("users").update({username: username , 'match.name': {$ne: currentUser.username}}, {$addToSet: {match: {name: currentUser.username, date: dateFormat(), time: Date.now(), type: "match", photo: currentUser.photo, room: room[0] + room[1]}}});
        req.app.db.collection("users").update({username: currentUser.username , 'match.name': {$ne: username}}, {$addToSet: {match: {name: username, date: dateFormat(), time: Date.now(), type: "match", photo: photo, room: room[0] + room[1]}}, $set: {notif: 1}});
        break ;
      }
    }}
    //REMOVE DISLIKE
...
router.post('/dislike', middleware.loggedIn(), (req, res)=>{
  var currentUser = {username: req.cookies.user.username, photo: req.cookies.user.photo},
      ...
      profile.toArray().then((profile)=>{
        ...
        //ADD dislike
          req.app.db.collection("users").update({username: username}, {$addToSet: {dislike: {name: currentUser.username, date: dateFormat(), type: "dislike", photo: currentUser.photo}}});
        }
      });
});
