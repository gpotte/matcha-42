...
$('#signInForm').submit((ev)=>{
  ...
  ev.preventDefault();
    $.ajax({
      ...
      url: 'http://9a4994d1.ngrok.io/user/new',
      success: function(data) {
        ...
        if (data === "Success")
          $(location).attr('href', "http://9a4994d1.ngrok.io/");
        console.log(data);
      }
    });
});
...
$('#logInForm').submit((ev)=>{
  ...
  ev.preventDefault();
    $.ajax({
      ...
      data: JSON.stringify(data),
      contentType: 'application/json',
      url: 'http://9a4994d1.ngrok.io/login',
      success: function(data) {
        if (data === "Error")
        {
          $(".container").prepend("<div class='alert alert-danger alert-dismissable fade in'>\
...
$('#lostForm').submit((ev)=>{
  ...
  ev.preventDefault();
    $.ajax({
      ...
      data: JSON.stringify(data),
      contentType: 'application/json',
      url: 'http://9a4994d1.ngrok.io/login/lost',
      success: function(data) {
        if (data === "Error")
        {
          $(".container").prepend("<div class='alert alert-danger alert-dismissable fade in'>\
...
$('#resetForm').submit((ev)=>{
  ...
  ev.preventDefault();
    $.ajax({
      ...
      data: JSON.stringify(data),
      contentType: 'application/json',
      url: 'http://9a4994d1.ngrok.io/login/newPassword',
      success: function(data) {
        if (data === "Error")
        {
          $(".container").prepend("<div class='alert alert-danger alert-dismissable fade in'>\
